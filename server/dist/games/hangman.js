...
exports.getHangmanSession = getHangmanSession;
exports.saveHangmanSession = saveHangmanSession;
exports.deleteHangmanSession = deleteHangmanSession;
exports.getMaskedHangmanState = getMaskedHangmanState;
exports.processHangmanMove = processHangmanMove;
const framework_1 = require("./framework");
async function getHangmanSession(roomCode, roomId, prisma) {
    return (0, framework_1.getGameSessionFromCache)('hangman', roomCode, roomId, prisma);
}
async function saveHangmanSession(roomCode, state) {
    return (0, framework_1.saveGameSessionToCache)('hangman', roomCode, state);
}
async function deleteHangmanSession(roomCode) {
    return (0, framework_1.deleteGameSessionFromCache)('hangman', roomCode);
}
/**
 * Masks the secret words for security so players cannot inspect/cheat.
 */
function getMaskedHangmanState(state, targetUserId) {
    if (!state)
        return null;
    // Deep copy the state
    ...
    if (state.stage === 'WORD_SUBMISSION') {
        ...
        if (isP1) {
            masked.p2Word = state.p2Word
                ? state.p2Word.split('').map((c) => state.p1Guesses.includes(c) ? c : '_').join('')
                : '';
        }
        ...
        // P2 entered p2Word, so P2 is allowed to see p2Word.
            ...
            masked.p1Word = state.p1Word
                ? state.p1Word.split('').map((c) => state.p2Guesses.includes(c) ? c : '_').join('')
                : '';
        }
        else {
            // Spectator
...
async function processHangmanMove(roomCode, roomId, userId, move, players, prisma) {
    ...
    if (currentGameState.stage === 'WORD_SUBMISSION') {
        ...
        // Persist and save
        (0, framework_1.persistSnapshotAsync)(roomId, currentGameState, 'PLAYING', null, nextTurn, 'hangman', prisma);
    ...
    if (currentGameState.stage === 'PLAYING') {
        ...
        if (move.type === 'GUESS_LETTER') {
            ...
            const solved = targetWord.split('').every((c) => guessedList.includes(c));
    ...
    if (gameFinished && winnerId) {
        ...
        (0, framework_1.persistSnapshotAsync)(roomId, currentGameState, updatedStatus, winnerId, nextTurn, 'hangman', prisma);
