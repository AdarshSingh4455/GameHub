...
exports.MOCK_COSMETIC_ITEMS = void 0;
exports.loadDb = loadDb;
exports.saveDb = saveDb;
exports.createPrismaMockProxy = createPrismaMockProxy;
const path_1 = __importDefault(require("path"));
const fs_1 = __importDefault(require("fs"));
const DB_PATH = path_1.default.join(process.cwd(), 'node_modules', '.cache', 'mock_db_state.json');
exports.MOCK_COSMETIC_ITEMS = [
    // Avatars
...
function loadDb() {
    ...
    while (retries > 0) {
        try {
            if (fs_1.default.existsSync(DB_PATH)) {
                const content = fs_1.default.readFileSync(DB_PATH, 'utf8');
                if (content.trim()) {
                    const parsed = JSON.parse(content);
                    if (parsed.profiles && parsed.rooms && parsed.sessions && parsed.friendships) {
                        if (!parsed.inventories)
                            parsed.inventories = {};
                        if (!parsed.challengeClaims)
                            parsed.challengeClaims = {};
                        if (!parsed.matches)
                            parsed.matches = {};
                        if (!parsed.xpEvents)
                            parsed.xpEvents = {};
    ...
    try {
        ...
        fs_1.default.writeFileSync(tmpPath, JSON.stringify(defaultDb, null, 2), 'utf8');
        fs_1.default.renameSync(tmpPath, DB_PATH);
    }
    catch (e) { }
    return defaultDb;
}
function saveDb(state) {
    ...
    while (retries > 0) {
        try {
            ...
            if (!fs_1.default.existsSync(dir))
                fs_1.default.mkdirSync(dir, { recursive: true });
            fs_1.default.writeFileSync(tmpPath, JSON.stringify(state, null, 2), 'utf8');
            fs_1.default.renameSync(tmpPath, DB_PATH);
            return;
        }
        catch (err) {
...
function getOrCreateProfile(db, userId, overrideName) {
    ...
    if (profile.selectedTitle === undefined)
        profile.selectedTitle = null;
    if (profile.selectedBadge === undefined)
    ...
    if (profile.selectedEffect === undefined)
        profile.selectedEffect = null;
    if (profile.currentRank === undefined)
        profile.currentRank = null;
    if (profile.previousRank === undefined)
        profile.previousRank = null;
    if (profile.pinnedAchievements === undefined)
        profile.pinnedAchievements = [];
    if (profile.streakProtectionActive === undefined)
        profile.streakProtectionActive = false;
    if (profile.lastActiveAt === undefined)
        profile.lastActiveAt = new Date().toISOString();
    if (profile._count === undefined)
        profile._count = { wonMatches: 0, friends: 0 };
    return profile;
}
// ─── Model Mock Factory ──────────────────────────────────────────────────────
            ...
            return async (...args) => {
                ...
                if (modelName === 'profile') {
                    if (action === 'findUnique' || action === 'findFirst') {
                        ...
                        if (username)
                            return Object.values(db.profiles).find(p => p.username === username) || null;
                        if (friendCode)
                            return Object.values(db.profiles).find(p => p.friendCode === friendCode) || null;
                        return null;
                    }
                    if (action === 'upsert') {
                        const db = loadDb();
                    ...
                    if (action === 'update') {
                        ...
                        if (profile) {
                            const d = params.data || {};
                            if (d.xp?.increment)
                                profile.xp += d.xp.increment;
                            if (d.coins?.increment)
                                profile.coins += d.coins.increment;
                    ...
                    if (action === 'findMany') {
                        ...
                        if (params.select) {
                            ...
                            list = list.map(p => {
                                ...
                                if (p.selectedTitle === undefined)
                                    p.selectedTitle = null;
                                if (p.selectedBadge === undefined)
                                ...
                                if (p.selectedEffect === undefined)
                                    p.selectedEffect = null;
                                if (p.currentRank === undefined)
                                    p.currentRank = null;
                                if (p.previousRank === undefined)
                                    p.previousRank = null;
                                if (p._count === undefined)
                                    p._count = { wonMatches: 0, friends: 0 };
                ...
                if (modelName === 'friendship') {
                    if (action === 'findMany') {
                        ...
                        if (where.OR) {
                            const ids = where.OR.flatMap((c) => Object.values(c));
                            list = list.filter(f => ids.includes(f.requesterId) || ids.includes(f.addresseeId));
                        }
                        if (where.requesterId)
                            list = list.filter(f => f.requesterId === where.requesterId);
                        if (where.addresseeId)
                            list = list.filter(f => f.addresseeId === where.addresseeId);
                        if (where.status)
                    ...
                    if (action === 'findFirst') {
                        ...
                        return Object.values(db.friendships).find(f => {
                            if (where.OR) {
                                return where.OR.some((c) => Object.entries(c).every(([k, v]) => f[k] === v));
                            }
                    ...
                    if (action === 'upsert') {
                        ...
                        let existing = null;
                        if (where.id)
                            existing = db.friendships[where.id];
                ...
                if (modelName === 'multiplayerInvite') {
                    if (action === 'findMany') {
                        ...
                        if (where.receiverId)
                            list = list.filter(i => i.receiverId === where.receiverId);
                        if (where.senderId)
                            list = list.filter(i => i.senderId === where.senderId);
                        if (where.status)
                    ...
                    if (action === 'findFirst') {
                        const db = loadDb();
                        const where = params.where || {};
                        return Object.values(db.invites).find(i => Object.entries(where).every(([k, v]) => i[k] === v)) || null;
                    }
                    if (action === 'findUnique') {
                        const db = loadDb();
                    ...
                    if (action === 'update' || action === 'updateMany') {
                        ...
                        if (!id && params.where?.roomId && params.where?.receiverId) {
                            const list = Object.values(db.invites).filter((i) => i.roomId === params.where.roomId && i.receiverId === params.where.receiverId);
                            list.forEach((i) => Object.assign(i, params.data || {}));
                            saveDb(db);
                ...
                if (modelName === 'notification') {
                    ...
                    if (action === 'findMany') {
                        ...
                        if (where.profileId)
                            list = list.filter(n => n.profileId === where.profileId);
                        if (where.userId)
                    ...
                    if (action === 'findFirst') {
                        ...
                        if (where.profileId)
                            list = list.filter(n => n.profileId === where.profileId);
                        if (where.type)
                            list = list.filter(n => n.type === where.type);
                ...
                if (modelName === 'multiplayerRoom') {
                    if (action === 'findUnique') {
                        ...
                        if (id)
                            return Object.values(db.rooms).find(r => r.id === id) || null;
                        return null;
                    }
                    if (action === 'findMany') {
                        const db = loadDb();
                    ...
                    if (action === 'findFirst') {
                        const db = loadDb();
                        const where = params.where || {};
                        return Object.values(db.rooms).find(r => Object.entries(where).every(([k, v]) => {
                            if (k === 'OR')
                                return v.some(c => Object.entries(c).every(([ck, cv]) => (r.players || []).some((p) => p[ck] === cv)));
                            return r[k] === v;
                        })) || null;
                    }
                    if (action === 'create') {
                ...
                if (modelName === 'multiplayerRoomPlayer') {
                    ...
                    if (action === 'findFirst') {
                        ...
                        for (const room of Object.values(db.rooms)) {
                            const p = (room.players || []).find((p) => Object.entries(where).every(([k, v]) => p[k] === v || room[k] === v));
                            if (p)
                                return { ...p, room };
                        }
                        return null;
                    ...
                    if (action === 'create') {
                        ...
                        if (room && !room.players.some((p) => p.userId === d.userId)) {
                                ...
                                for (const [key, val] of Object.entries(where)) {
                                    if (key === 'NOT') {
                                        ...
                                        for (const [nk, nv] of Object.entries(val)) {
                                            if (nk === 'status' && p.status !== nv)
                                                notMatches = false;
                                            if (nk === 'userId' && p.userId !== nv)
                                                notMatches = false;
                                            if (nk === 'roomId' && p.roomId !== nv && room.id !== nv)
                                                notMatches = false;
                                        }
                                        if (notMatches)
                                            matches = false;
                ...
                if (modelName === 'multiplayerGameSession') {
                    if (action === 'findUnique') {
                        ...
                        if (roomId)
                            return db.sessions[roomId] || null;
                        if (id)
                            return Object.values(db.sessions).find(s => s.id === id) || null;
                        return null;
                    }
                    if (action === 'create') {
                        const db = loadDb();
                ...
                if (modelName === 'cosmeticItem') {
                    if (action === 'findMany') {
                        const where = params.where || {};
                        let list = [...exports.MOCK_COSMETIC_ITEMS];
                        if (where.type) {
                            if (where.type.in) {
                    ...
                    if (action === 'findUnique' || action === 'findFirst') {
                        ...
                        if (id)
                            return exports.MOCK_COSMETIC_ITEMS.find(item => item.id === id) || null;
                        if (name)
                            return exports.MOCK_COSMETIC_ITEMS.find(item => item.name === name) || null;
                        return null;
                    }
                }
                // ── ProfileInventory ──────────────────────────────────────────────────
                if (modelName === 'profileInventory') {
                    if (action === 'findMany') {
                        ...
                        if (where.profileId) {
                            list = list.filter((i) => i.profileId === where.profileId);
                        }
                        list = list.map((i) => ({
                            ...i,
                            cosmeticItem: exports.MOCK_COSMETIC_ITEMS.find(c => c.id === i.cosmeticItemId) || null
                        }));
                        return list;
                    }
                    if (action === 'findUnique' || action === 'findFirst') {
                        ...
                        if (id) {
                            ...
                            if (res) {
                                res.cosmeticItem = exports.MOCK_COSMETIC_ITEMS.find(c => c.id === res.cosmeticItemId) || null;
                            }
                            return res;
                        }
                    ...
                    if (action === 'create') {
                        ...
                        return {
                            ...invRecord,
                            cosmeticItem: exports.MOCK_COSMETIC_ITEMS.find(c => c.id === data.cosmeticItemId) || null
                        };
                    }
                }
                // ── ChallengeClaim ────────────────────────────────────────────────────
                if (modelName === 'challengeClaim') {
                    if (action === 'findMany') {
                        ...
                        if (where.profileId) {
                            list = list.filter((c) => c.profileId === where.profileId);
                        }
                        return list;
                    }
                    if (action === 'findUnique' || action === 'findFirst') {
                ...
                if (modelName === 'tournament') {
                    ...
                    if (action === 'findUnique' || action === 'findFirst') {
                        const db = loadDb();
                        const id = params.where?.id;
                        if (id && db.tournaments)
                            return db.tournaments[id] || null;
                        return null;
                    }
                    if (action === 'create') {
                        const db = loadDb();
                ...
                if (modelName === 'matchRecord') {
                    ...
                    if (action === 'findMany') {
                        ...
                        if (where.OR) {
                            list = list.filter((m) => {
                                return where.OR.some((cond) => {
                                    if (cond.player1Id && m.player1Id !== cond.player1Id)
                                        return false;
                                    if (cond.player2Id && m.player2Id !== cond.player2Id)
                                        return false;
                                    return true;
                                });
                            });
                        ...
                        if (where.playedAt?.gte) {
                            ...
                            list = list.filter((m) => new Date(m.playedAt) >= gteDate);
                        }
                        if (where.winnerId) {
                            list = list.filter((m) => m.winnerId === where.winnerId);
                        }
                        // Sorting / take
                ...
                if (modelName === 'xPEvent') {
                    ...
                    if (action === 'findMany') {
                        ...
                        if (where.profileId) {
                            list = list.filter((x) => x.profileId === where.profileId);
                        }
                        if (where.createdAt?.gte) {
                            const gteDate = new Date(where.createdAt.gte);
                            list = list.filter((x) => new Date(x.createdAt) >= gteDate);
                        }
                        return list;
                    }
                    if (action === 'aggregate') {
                ...
                if (action === 'updateMany')
                    return { count: 0 };
                if (action === 'aggregate')
                    return { _sum: { score: 0, amount: 0 }, _count: 0 };
                if (action === 'groupBy')
                    return [];
                return null;
            };
...
function createPrismaMockProxy(realPrisma) {
    const handler = {
        get(target, prop) {
            if (prop === '$queryRaw')
