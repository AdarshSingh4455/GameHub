...
import { WORD_CATEGORIES, LOCAL_CATEGORIES } from './words'

export const SCRIBBLE_WORDS = LOCAL_CATEGORIES.Animals

/**
 * Generates word lists via the Groq API
 */
async function generateGroqWords(category: string): Promise<string[]> {
 ...
 */
async function fetchAndAppendWords(roomCode: string, roomId: string, category: string, prisma: any) {
  try {
    logger.info(`[Scribble Groq] Initiating background word generation for room ${roomCode}, category ${category}`)
    ...
    if (addedWords.length > 0) {
      ...
      logger.info(`[Scribble Groq] Successfully appended ${addedWords.length} new Groq words to room ${roomCode}`)
    ...
    logger.error(`[Scribble Groq] Failed to fetch background words for room ${roomCode}: ${err.message}`)
  }
}
...
export async function processScribbleMove(
  ...
  if (type === 'settings') {
    ...
    const localWords = LOCAL_CATEGORIES[category] || LOCAL_CATEGORIES.Animals
    ...
    fetchAndAppendWords(roomCode, roomId, category, prisma).catch(err => {
      logger.error(`[Scribble Groq] Start prefetch background error: ${err.message}`)
    })
  ...
  if (state.round > state.maxRounds) {
    ...
    if (state.unusedWords.length < 20) {
      fetchAndAppendWords(roomCode, roomId, state.category, prisma).catch(err => {
        logger.error(`[Scribble Groq] Turn prefetch background error: ${err.message}`)
      })
    }
