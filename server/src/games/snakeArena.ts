...
export const snakeArenaTickIntervals = new Map<string, NodeJS.Timeout>()

export async function getSnakeArenaSession(roomCode: string, roomId: string, prisma: any): Promise<SnakeArenaState | null> {
  return getGameSessionFromCache('snake-arena', roomCode, roomId, prisma)
}

export async function saveSnakeArenaSession(roomCode: string, state: SnakeArenaState): Promise<void> {
  return saveGameSessionToCache('snake-arena', roomCode, state)
}

export async function deleteSnakeArenaSession(roomCode: string): Promise<void> {
  return deleteGameSessionFromCache('snake-arena', roomCode)
}

// Spawns food at a safe position not occupied by snake heads
function spawnFoodAtRandom(cols: number, rows: number, snakes: Record<string, SnakePlayer>, type: 'normal' | 'golden' | 'giant'): FoodItem {
  let attempts = 0
  ...
  let value = 10
  if (type === 'golden') value = 30
  if (type === 'giant') value = 20
...
function spawnPowerupAtRandom(cols: number, rows: number, snakes: Record<string, SnakePlayer>): PowerupItem {
...
export function processSnakeDirectionChange(
  state: SnakeArenaState,
  userId: string,
  newDir: Position
): SnakeArenaState {
  const snake = state.snakes[userId]
  if (!snake || snake.status !== 'ACTIVE') return state
...
export function processSnakeBoost(
  state: SnakeArenaState,
  userId: string,
  isBoosting: boolean
): SnakeArenaState {
...
export function tickSnakeArena(state: SnakeArenaState): { state: SnakeArenaState; events: any[] } {
  ...
  if (state.tickCount % 50 === 0 && state.foods.filter(f => f.type !== 'normal').length < 3) {
    ...
    state.foods.push(spawnFoodAtRandom(state.cols, state.rows, state.snakes, fType as any))
  }
  ...
  const eliminatedSnakes: string[] = []
  const eliminationsLog: { deadId: string; killerId: string | null }[] = []
  ...
  for (const snake of activeSnakes) {
    ...
    if (!isSpawnProtected && !hasGhost) {
      for (const other of activeSnakes) {
        ...
        if (otherProtected) continue // Ignore protected snakes
  ...
  const headLocations: Record<string, string[]> = {}
  for (const snake of activeSnakes) {
    if (eliminatedSnakes.includes(snake.userId)) continue
    const body = nextBodies[snake.userId]
    if (!body || body.length === 0) continue
    const head = body[0]
    const key = `${head.x},${head.y}`
    if (!headLocations[key]) headLocations[key] = []
  ...
  for (const snake of activeSnakes) {
    ...
    if (eatenIndexes.length > 0) {
      ...
      for (const idx of eatenIndexes) {
        ...
        let growAmount = 1
        if (food.type === 'giant') growAmount = 3
        if (hasDouble) growAmount *= 2
...
export function startSnakeArenaTick(roomCode: string, io: any, prisma: any) {
  ...
  const interval = setInterval(async () => {
    try {
      ...
      if (nextState.status === 'FINISHED') {
        stopSnakeArenaTick(roomCode)
        persistSnapshotAsync(room.id, nextState, 'FINISHED', nextState.winnerId, null, 'snake-arena', prisma)
        await handleMatchCompletion(room, nextState, nextState.winnerId, prisma)
      }
    } catch (err: any) {
      logger.error(`Error in snake-arena tick for roomCode=${roomCode}: ${err?.message || err}`)
    }
  }, 100)

  snakeArenaTickIntervals.set(roomCode, interval)
}

export function stopSnakeArenaTick(roomCode: string) {
  const interval = snakeArenaTickIntervals.get(roomCode)
  if (interval) {
    clearInterval(interval)
    snakeArenaTickIntervals.delete(roomCode)
  }
}
