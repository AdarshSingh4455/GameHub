...
import { LOCAL_CATEGORIES, WORD_CATEGORIES, WORD_ALIASES } from './words'
import { getRoomQueue, deleteRoomQueue } from '../utils/queue'
import { userSockets } from '../index'
import { INITIAL_STATES } from './framework'

const GAME_CACHE_TTL = 7200
const whosSpyTimers = new Map<string, NodeJS.Timeout>()

export async function getWhosSpySession(roomCode: string, roomId: string, prisma: any): Promise<any> {
...
export async function saveWhosSpySession(roomCode: string, state: any): Promise<void> {
  const redisKey = `game:whosspy:${roomCode}`
  if (redisClient.isReady) {
    try {
...
export async function deleteWhosSpySession(roomCode: string): Promise<void> {
  const redisKey = `game:whosspy:${roomCode}`
  if (redisClient.isReady) {
    try {
...
export function clearWhosSpyTimer(roomCode: string) {
...
export function isWordBlocked(message: string, secretWord: string): boolean {
  ...
  const aliases = WORD_ALIASES[secretWord] || []
...
export function getWhosSpyMaskedState(state: any, userId: string): any {
...
export function broadcastGameState(roomCode: string, state: any, players: any[], io: any) {
...
export function startWhosSpyTimer(
  roomCode: string,
  roomId: string,
  durationMs: number,
  targetStage: 'VOTING' | 'VOTE_REVEAL',
  players: any[],
  prisma: any,
  io: any
) {
...
async function resolveVotes(roomCode: string, roomId: string, state: any, players: any[], prisma: any, io: any) {
  clearWhosSpyTimer(roomCode)
...
export async function handleWhosSpyCompletionRewards(
  roomId: string,
  state: any,
  ...
  try {
    ...
    logger.error(err, 'Failed to distribute Whos Spy rewards')
  }
}

export async function processWhosSpyMove(
  roomCode: string,
  roomId: string,
  userId: string,
  move: any,
...
export async function startWhosSpyRematch(roomCode: string, hostUserId: string, players: any[], prevGameState: any): Promise<any> {
  await deleteWhosSpySession(roomCode)
  const prevUsedWords = prevGameState?.usedWords || {}
  const finalGameState = INITIAL_STATES['whos-spy'](players, hostUserId)
  
  const category = finalGameState.category
  const used = prevUsedWords[category] || []
  const allWords = LOCAL_CATEGORIES[category] || LOCAL_CATEGORIES.Animals
  let unusedWords = allWords.filter((w: string) => !used.includes(w))
